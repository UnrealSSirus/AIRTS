"""Hard AI — adaptive strategy with reactive builds, per-unit micro, and retreat logic."""
...
class HardAI(BaseAI):
    ...
    def _update_build(self, scouts, tanks, snipers, medics, shockwaves,
        ...
        if dominant in counter:
            self._set_build_once(counter[dominant])
            return
    ...
    def _should_retreat(self, unit, cc) -> bool:
        return unit.hp < unit.max_hp * 0.75
    ...
    def _command_snipers(self, snipers, enemies, enemy_mobile, cc, mid_x):
        ...
        for sniper in snipers:
            ...
            if non_sniper_mobile:
                ...
                self._sniper_kite(sniper, target, enemy_mobile, cc)
            elif enemy_mobile:
                # Only snipers remain
